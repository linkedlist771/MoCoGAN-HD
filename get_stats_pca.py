...
class IPCAEstimator:
    def __init__(self, n_components):
        ...
        self.transformer = IncrementalPCA(
            n_components, whiten=self.whiten, batch_size=max(100, 2 * n_components)
        )
        self.fit_time = None
...
def main():
    ...
    with torch.no_grad():
        for _ in tqdm(
            range(args.pca_iterations), desc="Computing PCA", unit="iteration"
        ):
            z.data.normal_()
        ...
        np.save(os.path.join(args.save_pca_path, "pca_var_ratio.npy"), var_ratio)
