...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.rank % ngpus_per_node == 0:
        writer = SummaryWriter(log_dir=os.path.join(args.checkpoints_dir, "runs"))

    total_steps = 0
    train_func.toggle_grad(modelG, False)

    for epoch in tqdm(
        range(args.load_pretrain_epoch + 1, args.total_epoch), desc="Epochs"
    ):
        if args.distributed:
        ...
        for step, data in enumerate(tqdm(dataset, desc=f"Epoch {epoch}", leave=False)):
            ...
            total_steps += 1
            loss_all, loss_names = train_func.GD_step(
                args, modelG, modelD_img, modelD_3d, data, x, z
            )
            ...
            if total_steps % args.save_latest_freq == 0 and (
                ...
                save_models(
                    modelG, modelD_img, modelD_3d, args.checkpoints_dir, "latest"
                )
...
def save_models(modelG, modelD_img, modelD_3d, ckpt_dir, string):
    ...
    ckpt = {"g_ema": modelG.state_dict()}
    torch.save(ckpt, "%s/modelG_%s.pth" % (ckpt_dir, string))

    torch.save(
        modelG.module.modelR.state_dict(), "%s/modelR_%s.pth" % (ckpt_dir, string)
    )

    torch.save(modelD_img.state_dict(), "%s/modelD_img_%s.pth" % (ckpt_dir, string))
    torch.save(modelD_3d.state_dict(), "%s/modelD_3d_%s.pth" % (ckpt_dir, string))
...
def save_videos(writer, z, modelG, string, args):
    with torch.no_grad():
        ...
        writer.add_video(string, (x_fake.data + 1.0) / 2.0)
        modelG.train()
